...
export DOCKER_HUB_LOGIN=registry.blockstate.com:5000 #TODO remove
...
selectSlave() {
	if [ "$REMOTE" != "n" ]
	then
		export DOCKER_HOST="tcp://35.242.228.181:2376"
		export DOCKER_ID_USER="reg"
		export DOCKER_TLS_VERIFY="1"
		export DOCKER_CERT_PATH=~/.docker/dockers2
		echo switching to $DOCKER_HOST
	fi
}
